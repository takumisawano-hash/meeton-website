...
| # | Module | Body |
|---|--------|------|
| 01 | **Meeton Email** (AI フォロー) | CRM 内の過去 MQL・失注 contact の検討再開シグナル (Web 再訪、料金ページ閲覧、メール開封) を検知し、AI が 1:1 文脈に沿った再アプローチを **支援 / 自動化**。MA 一斉送信ではなく動的に内容・タイミング・トーンを判断します。必要に応じて、送信前に承認フローを挟む運用も選べます。 → `/features/ai-email/` |
...
| # | Title | Body |
|---|-------|------|
| 01 | CRM / MA を接続 | HubSpot / Salesforce / Marketo のネイティブ連携でリード・会話履歴・商談履歴を取り込み。**最短 30 分〜**、初期設定は DynaMeet 担当者が支援。顧客環境により所要時間は変動。 |
...
1. **CRM 連携の工数はどのくらいですか？**
   HubSpot / Salesforce はネイティブ連携済み、OAuth ベースで最短 30 分〜
   数時間で接続できます。顧客環境 (カスタム項目数 / 権限構成) によって
   差は出ますが、初期設定は DynaMeet 担当者が伴走するため技術専任
   エンジニアは不要です。Marketo / Pardot は API 連携で数日が目安。
...
5. **既存 contact に勝手にメールが送られて炎上しませんか？**
   ...
   contact は除外。送信前に必ず人間のレビュー段階を入れる承認フロー
   運用も選べます。

6. **セキュリティはどうなっていますか？**
   ISMS 認証は審査合格済み・認証発行待ちの段階です。顧客データは
   AES-256 で保管、社内アクセスは最小権限。HubSpot / Salesforce との
   連携も official OAuth scope のみを要求し、データ書き戻し範囲は管理
   画面で個別 ON/OFF できます。**セキュリティチェックシート対応、
   NDA 締結、アクセス権限管理** にも個別対応可能。詳細は情報セキュリティ
...
**LP launch と同時に DL 可能な状態にする**: 初版は 3〜5 ページの簡易版
で OK、ブランド整合だけ確保。後で v2 で内容を厚くする予定。
...
## 17. Open questions (resolved 2026-05-23)

すべて takumi より回答済み:

1. **チェックリスト PDF**: LP launch と同時に DL 可能にする。初版は
   3-5 ページ簡易版で OK、後追いで v2 拡張。

2. **Meeton Email reactivation**: **実装済み**。spec で「支援 / 自動化、
   承認フロー選択可」の表現に softening 済 (絶対断言は避ける、必要に
   応じて人間 review を挟める運用も明示)。

3. **HubSpot / Salesforce 連携**: **連携済み**。spec で「最短 30 分〜、
   顧客環境差あり、初期設定は DynaMeet 担当者が伴走」と補足済。

4. **ISMS posture**: **審査合格済 / 認証発行待ち**。spec の FAQ 6 で
   「ISMS 認証は審査合格済み・認証発行待ちの段階」と修正済。FAQ 6 に
   セキュリティチェックシート / NDA / アクセス権限管理 への対応も
   追記済。
