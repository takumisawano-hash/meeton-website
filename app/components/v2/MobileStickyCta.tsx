...
import { demoUrl, openDemoCalendarInPlace } from "@/app/lib/cta-urls";
...
export default function MobileStickyCta({ lang = "ja", source }: { lang?: Lang; source: string }) {
  ...
  const trialHref = useTrialHref(`${source}-sticky`);
  const href = en ? trialHref : demoUrl(`${source}-sticky`);
  ...
  return (
    ...
    >
      ...
      <a
        ...
        onClick={(e) => {
          track();
          if (en) trackStartTrialClick(`${source}-sticky`);
