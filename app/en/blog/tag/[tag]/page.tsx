...
// /en/blog/tag/[tag]/ — English twin of /blog/tag/[tag]/. Resolves against EN
// posts only (lang='en'); links to /en/blog/*. Needed because the EN hub + EN
// detail pages link to tag archives.
...
async function resolveTag(slug: string): Promise<{ name: string; count: number } | null> {
  const tags = await getTagsWithCounts(1, 'en')
  ...
  const fallback = tags.find((t) => t.name.toLowerCase().trim() === decoded.toLowerCase().trim())
...
export async function generateMetadata({
  ...
  if (!t) return {}
  const title = `#${t.name} posts`
  const description = `Meeton ai blog posts tagged “${t.name}”. Concrete tactics and examples for AI SDRs, B2B sales and inside sales.`
  return {
    title,
    description,
    ...
    openGraph: {
      title,
      description,
      url: `https://dynameet.ai/en/blog/tag/${tag}/`,
      siteName: 'Meeton ai',
      locale: ogLocale('en'),
      type: 'website',
    },
...
export default async function TagPageEn({
  ...
  const [posts, allTags] = await Promise.all([
    getPostsByTag(t.name, 'en'),
    getTagsWithCounts(5, 'en'),
  ])
  ...
  const jsonLd = {
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 3, name: `#${t.name}`, item: `https://dynameet.ai/en/blog/tag/${tag}/` },
      ],
    },
  }
  ...
  return (
    <div style={{ background: '#fafaf7', color: '#0a0e0c', minHeight: '100vh' }}>
      ...
      >
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <p style={{ fontSize: 16, lineHeight: 1.85, color: '#3d4541', maxWidth: 640, margin: 0 }}>
            {t.count} posts tagged “{t.name}”. A collection of concrete know-how on related topics.
          </p>
      ...
      <section style={{ padding: 'clamp(48px, 6vw, 80px) clamp(20px, 4vw, 48px)' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          {posts.length === 0 ? (
            <p style={{ color: '#82897f' }}>There are no posts with this tag yet.</p>
          ) : (
            <div
              style={{
                display: 'grid',
