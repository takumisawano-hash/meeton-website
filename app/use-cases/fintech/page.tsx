    ...
    <UseCasePageClient
      ...
      modules={[
        ...
        },
        {
          ...
          name: 'Meeton Ads',
          href: '/ads/',
          description: 'サイト訪問者ごとに、閲覧ページ・流入元・業種に応じて最適なオファーを AI が出し分けるオンサイト広告。クリック・リード獲得から継続的に学習します。',
          industryAngle: '銀行・証券・保険など業種の異なる技術バイヤーに、閲覧ページ・流入元に応じた最適オファー（資料DL / 技術相談）を AI が提示。チャットを開かない訪問者もオンサイト広告でリード化します。広告文は AI が下書きし、コンプライアンス確認のうえ承認したものだけを配信します。',
        },
      ]}
      integrations={[
        {
      ...
      faqs={[
        {
          ...
          answer: 'Meeton Chat はシステムプロンプトに「使用禁止表現リスト」を組み込み、出力前にコンプライアンスチェックを通します。さらに、貴社のコンプラ担当者が承認した表現テンプレートのみを使用するよう設定可能です。導入時に貴社のコンプライアンス要件をヒアリングし、初期プロンプトを共同設計します。',
        },
        {
          question: '顧客側の規制ステータス（銀行・証券・保険など）に応じた提供可否判定を、AI で実行できますか？',
          answer: 'Meeton Chat が問い合わせ過程で「業種・ライセンス区分・地域」を確認し、貴社の対応可能セグメントマトリクスと照合します。対応不可セグメントは丁寧な案内のみとし、対応可能リードのみを商談化に進めます。営業現場で初めて不適合が判明する無駄を排除できます。',
        },
        {
          question: 'API 仕様・SLA・冗長性・暗号化方式など、技術リードからの深い質問に AI で対応できますか？',
          answer: 'はい。Meeton Chat は貴社の技術ドキュメント（API リファレンス、セキュリティホワイトペーパー、SOC2 / ISMS 報告書のサマリー）を学習データに含めることができます。技術リードが投げる API 仕様・SLA・冗長性・暗号化アルゴリズムなどの質問に、貴社の公式情報を根拠に深く回答可能です。',
        },
        {
