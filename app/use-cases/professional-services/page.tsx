...
export default function ProfessionalServicesUseCasePage() {
  return (
    <UseCasePageClient
      ...
      painsLead="リレーション営業が本質のプロフェッショナルサービスでは、パートナーの時間が最大のボトルネック。一方でインバウンドは案件選別の前段が必要です。"
      pains={[
        {
      ...
      faqs={[
        {
          ...
          answer: '一次ヒアリングを AI に任せる目的は、パートナーが「価値の高い対話」に集中するためです。Meeton Live は丁寧な言葉遣いで案件背景を確認し、有望案件はパートナーへ即座に引き継ぎます。コールド問い合わせには資料送付＋リソースガイドで対応し、適切な距離感を維持します。むしろ「初動が遅い」「秘書が機械的に対応する」よりも顧客満足度が上がるケースが多くあります。',
        },
        {
          question: '「相談したいだけ」のコールド問い合わせと、有望案件をどう仕分けますか？',
          answer: 'Meeton Live は問い合わせ過程で「予算感」「導入時期」「現状の課題」「意思決定者」などを自然な対話で確認し、案件スコアを自動算出します。スコアが基準を超えた問い合わせのみパートナーのカレンダーに直接予約され、それ以外は資料案内＋ナーチャー導線に乗せます。',
        },
        {
