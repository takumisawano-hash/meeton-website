...
:root {
  ...
  --fb:'Noto Sans JP','Plus Jakarta Sans',sans-serif;--fm:'JetBrains Mono',monospace;
}
...
.nav{position:fixed;top:0;left:0;right:0;z-index:100;padding:14px 48px;display:flex;align-items:center;justify-content:space-between;background:rgba(255,255,255,.88);backdrop-filter:blur(20px) saturate(1.4);border-bottom:1px solid rgba(226,230,240,.6)}
.nav-logo{font-weight:900;font-size:22px;color:var(--heading)}
.nav-logo span{color:var(--cta)}
.nav-links{display:flex;gap:28px}
.nav-links a{font-size:15px;color:var(--sub);text-decoration:none;font-weight:600;transition:color .2s}
...
.btn-outline{background:transparent;border:2px solid var(--border2);color:var(--sub);padding:12px 26px;border-radius:12px;font-size:15px;cursor:pointer;font-family:var(--fb);font-weight:700;transition:all .25s}
...
.footer a{font-size:13px;color:var(--sub);text-decoration:none;font-weight:600}
...
export default function App(){
  ...
  {/* NAV */}
  <nav className="nav">
    <div className="nav-logo">Meeton<span> Talent</span></div>
    <div className="nav-links"><a href="#sol">ソリューション</a><a href="#feat">機能</a><a href="#comp">比較</a><a href="#faq">FAQ</a></div>
    <div style={{display:"flex",gap:10}}><button className="btn btn-outline">資料請求</button><button className="btn btn-cta">デモを予約</button></div>
  </nav>
          ...
          {[
            ...
            <div className="fc-step">
              ...
              <div style={{flex:1}}>
                ...
                <div className="fc-bar"><div className="fc-fill" style={{background:s.c,"--sw":s.w,animationDelay:`${i*.3}s`}}/></div>
              </div>
              ...
              <div className="cc" style={{padding:"10px 12px"}}>
                ...
                <div className="cc-info">
                  ...
                  <div style={{height:4,borderRadius:2,background:"var(--surface2)",marginTop:4,overflow:"hidden"}}>
                    <div style={{height:"100%",borderRadius:2,background:c.color,"--sw":`${c.score}%`,animation:`scoreUp 1s ${c.delay} cubic-bezier(.16,1,.3,1) forwards`}}/>
                  </div>
                </div>
  ...
  <footer className="footer">
    <div style={{fontWeight:900,fontSize:17,color:"var(--sub)"}}>Meeton<span style={{color:"var(--cta)"}}> Talent</span></div>
    <div style={{display:"flex",gap:24}}><a href="#">プロダクト</a><a href="#">ブログ</a><a href="#">お問い合わせ</a></div>
